...
/// This also nils the error pointer before continuing, as most methods can't rely on a non-nil output to check if an
/// error occurred.
...
#define MP_READ_OBJECT_OR_RETURN(retVal)              \
    ...
    if (![self readObject:&msgpackObj error:error]) { \
        return retVal;                                \
    }
...
#define MP_ENFORCE_TYPE(_typeName, _typeInt)                            \
    ...
    if (msgpackObj.type != _typeInt) {                                  \
        if (error != nil) {                                             \
            *error = [self typeMismatchErrorForExpectedType:_typeName]; \
        }                                                               \
        return nil;                                                     \
    }
    ...
    return [NSError errorWithDomain:LOCAL_ERROR_DOMAIN
                           ...
                           userInfo:@{
                               NSLocalizedDescriptionKey :
                                   @"Type mismatch: underlying library supports the type, but our wrapper does not"
                           }];
}
    ...
    return [NSError errorWithDomain:LOCAL_ERROR_DOMAIN
                           ...
                           userInfo:@{
                               NSLocalizedDescriptionKey : [NSString
                                   stringWithFormat:@"Type mismatch: the expected type (%@) doesn't match the read one",
                                                    expectedType]
                           }];
}
...
- (nullable NSNumber *)readInteger:(bat_cmp_object_t)msgpackObj error:(NSError **)error allowNil:(BOOL)allowNil {
    MP_ENFORCE_TYPES(@"integer", 10, BAT_CMP_TYPE_POSITIVE_FIXNUM, BAT_CMP_TYPE_NEGATIVE_FIXNUM, BAT_CMP_TYPE_UINT8,
                     BAT_CMP_TYPE_UINT16, BAT_CMP_TYPE_UINT32, BAT_CMP_TYPE_UINT64, BAT_CMP_TYPE_SINT8,
                     BAT_CMP_TYPE_SINT16, BAT_CMP_TYPE_SINT32, BAT_CMP_TYPE_SINT64)
...
- (nullable NSNumber *)readDecimal:(bat_cmp_object_t)msgpackObj error:(NSError **)error allowNil:(BOOL)allowNil {
    MP_ENFORCE_TYPES(@"decimal", 2, BAT_CMP_TYPE_FLOAT, BAT_CMP_TYPE_DOUBLE)
...
- (nullable NSString *)readString:(bat_cmp_object_t)msgpackObj error:(NSError **)error allowNil:(BOOL)allowNil {
    MP_ENFORCE_TYPES(@"string", 4, BAT_CMP_TYPE_FIXSTR, BAT_CMP_TYPE_STR8, BAT_CMP_TYPE_STR16, BAT_CMP_TYPE_STR32)
...
- (nullable NSData *)readData:(bat_cmp_object_t)msgpackObj error:(NSError **)error allowNil:(BOOL)allowNil {
    MP_ENFORCE_TYPES(@"data", 3, BAT_CMP_TYPE_BIN8, BAT_CMP_TYPE_BIN16, BAT_CMP_TYPE_BIN32)
...
- (nullable NSDictionary *)readDictionary:(bat_cmp_object_t)msgpackObj error:(NSError **)error allowNil:(BOOL)allowNil {
    MP_ENFORCE_TYPES(@"dictionary", 3, BAT_CMP_TYPE_FIXMAP, BAT_CMP_TYPE_MAP16, BAT_CMP_TYPE_MAP32)
    ...
    for (uint32_t i = 0; i < dictionaryLength; i++) {
        ...
        if (![key conformsToProtocol:@protocol(NSCopying)]) {
            ...
            NSString *errorDescription =
                [NSString stringWithFormat:@"Dictionaries keys must conform to NSCopying: only MessagePack strings, "
                                           @"numbers and nil are supported. You will have to read this dictionary "
                                           @"manually if this is not a mistake. Key vas of type: '%s'",
                                           keyClassName];
...
- (nullable NSArray *)readArray:(bat_cmp_object_t)msgpackObj error:(NSError **)error allowNil:(BOOL)allowNil {
    MP_ENFORCE_TYPES(@"array", 3, BAT_CMP_TYPE_FIXARRAY, BAT_CMP_TYPE_ARRAY16, BAT_CMP_TYPE_ARRAY32)

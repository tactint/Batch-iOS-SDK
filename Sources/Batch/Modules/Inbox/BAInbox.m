...
- (void)setupUsingCache:(BOOL)useCache {
    ...
    if (useCache) {
        _fetcherId =
            [[BAInjection injectProtocol:@protocol(BAInboxDatasourceProtocol)] createFetcherIdWith:_clientType
...
- (void)fetchNewNotifications:(void (^_Nullable)(NSError *_Nullable error,
    ...
    dispatch_async(_dispatchQueue, ^{
      ...
      [self fetchFromWSForCursor:nil
                        callback:^(NSError *error, BAInboxWebserviceResponse *result) {
                          ...
                          if (error != nil) {
                              ...
                              NSError *err =
                                  [NSError errorWithDomain:LOCAL_ERROR_DOMAIN
                                                      code:-300
                              ...
                              NSError *err = nil;
                              NSArray<BAInboxNotificationContent *> *messages = [self handleResult:result
                                                                                 didAskNewMessages:true
                                                                                             error:&err];
                              if (completionHandler) {
                                  callbackToPerform = ^() {
                                    if (err) {
                                        ...
                                        completionHandler(nil, [self convertPrivateModelsToPublic:messages], false,
                                                          endReached);
                                    }
                                  };
                              }
      ...
      if (dispatch_semaphore_wait(waitSemaphore, dispatch_time(DISPATCH_TIME_NOW, 120 * NSEC_PER_SEC))) {
          [BALogger debugForDomain:DEBUG_DOMAIN
                           message:@"The semaphore waiting for the fetch expired. That should NOT happen."];
      }
    });
}
...
- (void)fetchNextPage:(void (^_Nullable)(NSError *_Nullable error,
                ...
                [NSError errorWithDomain:LOCAL_ERROR_DOMAIN
                                ...
                                userInfo:@{
                                    NSLocalizedDescriptionKey :
                                        @"The end of the inbox feed has been reached, either because the server has "
                                        @"nothing left to send or because the fetcher limit has been reached. You can "
                                        @"try bumping the limit using -[BatchInboxFetcher setLimit:]."
                                }];
    ...
    if (_cursor == nil) {
        [self fetchNewNotifications:^(NSError *error, NSArray<BatchInboxNotificationContent *> *notifications,
                                      BOOL foundNewMessages, BOOL endReached) {
          if (completionHandler) {
    ...
    dispatch_async(_dispatchQueue, ^{
      ...
      [self
          fetchFromWSForCursor:self->_cursor
                      callback:^(NSError *error, BAInboxWebserviceResponse *result) {
                        void (^callbackToPerform)(void) = NULL;
    ...
    @synchronized(_fetchedMessages) {
        ...
        if (internalNotification != nil) {
            ...
            [[BAInjection injectProtocol:@protocol(BAInboxDatasourceProtocol)]
                markAsRead:internalNotification.identifiers.identifier];
            internalNotification.isUnread = false;
            [notification _markAsRead];
        } else {
            [BALogger debugForDomain:DEBUG_DOMAIN
                             message:@"Could not find the specified notification (%@) to be marked as read",
...
- (void)markAllNotificationsAsRead {
    @synchronized(_fetchedMessages) {
        if ([_fetchedMessages count] > 0) {
            ...
            [[BAInjection injectProtocol:@protocol(BAInboxDatasourceProtocol)] markAllAsRead:(long long)now
...
- (void)markNotificationAsDeleted:(nonnull BatchInboxNotificationContent *)notification {
    ...
    @synchronized(_fetchedMessages) {
        ...
        if (internalNotification != nil) {
            ...
            [BALogger debugForDomain:DEBUG_DOMAIN
                             message:@"Could not find the specified notification (%@) to be marked as deleted",
                                     notification.identifier];
        }
    }
}
...
- (NSArray<NSObject *> *)allFetchedNotifications {
    // Someday, reimplement this by keeping a cache of the public models, alongside smarter deduplication of the first
    // refresh
...
- (void)fetchFromWSForCursor:(NSString *)cursor
                    callback:(void (^_Nonnull)(NSError *_Nullable error,
                                               BAInboxWebserviceResponse *_Nullable result))callback {
    ...
    if (_fetcherId != -1) {
        NSArray<BAInboxCandidateNotification *> *candidates = [[BAInjection
            injectProtocol:@protocol(BAInboxDatasourceProtocol)] candidateNotificationsFromCursor:cursor
...
- (void)syncFromWSForCursor:(NSString *)cursor
                 candidates:(NSArray<BAInboxCandidateNotification *> *)candidates
                   callback:(void (^_Nonnull)(NSError *_Nullable error,
                                              BAInboxWebserviceResponse *_Nullable result))callback {
...
- (NSArray<BAInboxNotificationContent *> *)handleResult:(BAInboxWebserviceResponse *)result
                                      didAskNewMessages:(BOOL)newMessages
                                                  error:(NSError **)error {
    @synchronized(_fetchedMessages) {
            ...
            if (error) {
                *error = [NSError
                    errorWithDomain:LOCAL_ERROR_DOMAIN
                               code:-101
                           userInfo:@{
                               NSLocalizedDescriptionKey : @"Server could not respond in time: no notifications have "
                                                           @"been fetched. Please retry later."
                           }];
            }
            return nil;
        }
        ...
        for (BAInboxNotificationContent *resMsg in result.notifications) {
            ...
            if (duplicateNotif != nil) {
                if ([resMsg.identifiers.identifier isEqualToString:duplicateNotif.identifiers.identifier]) {
                    [BALogger debugForDomain:DEBUG_DOMAIN
                                     message:@"Got the exact same notification twice. Skipping. (id %@)",
                                             resMsg.identifiers.identifier];
                } else {
                    [BALogger debugForDomain:DEBUG_DOMAIN
                                     message:@"Merging notifications for sendID %@ (identifiers: %@, %@)", sendID,
                                             resMsg.identifiers.identifier, duplicateNotif.identifiers.identifier];
...
- (NSArray<BatchInboxNotificationContent *> *)convertPrivateModelsToPublic:
    (NSArray<BAInboxNotificationContent *> *)privateModels {
    ...
    for (BAInboxNotificationContent *privateModel in privateModels) {
        ...
        if (model != nil) {
            ...
            [BALogger debugForDomain:DEBUG_DOMAIN message:@"Error while converting private model to public"];
        }
    }
...
- (NSArray<NSDictionary *> *)eventDatasForNotificationContent:(BAInboxNotificationContent *)content {
    ...
    NSMutableArray<BAInboxNotificationContentIdentifiers *> *contentIdentifiers =
        [NSMutableArray arrayWithObject:content.identifiers];

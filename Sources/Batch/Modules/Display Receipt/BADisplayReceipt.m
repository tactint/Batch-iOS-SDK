...
+ (nullable instancetype)unpack:(nonnull NSData *)data error:(NSError **)error {
    ...
    return [[BADisplayReceipt alloc] initWithTimestamp:[timestamp longLongValue]
                                                replay:replay
                                           sendAttempt:[sendAttempt intValue]
                                              openData:od
                                             eventData:ed];
}
...
+ (BOOL)checkError:(NSError *)error field:(NSString *)field {
    if (error != nil) {
        [BALogger errorForDomain:LOGGER_DOMAIN
                         message:@"Could not pack/unpack receipt field %@: %@", field, error.localizedDescription];
        return true;
    }
    return false;
}

...
/** Override to set up autoclosing UI elements before animation takes place. Default implementation strokes the close
 * button. */
- (void)setupAutoclosing;
/** Default to true. Override and return NO if countdown should wait for a condition, e.g. some initial loading to
 * finish. */
- (BOOL)automaticAutoclosingCountdown;
/** Manually start the autoclosing coundown. */
- (void)startAutoclosingCountdown;
/** Override to handle the animation your way. Default implementation configures the close button (if not nil) for
 * animation. */

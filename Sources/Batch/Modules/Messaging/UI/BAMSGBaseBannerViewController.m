...
- (void)setupViewContents {
    ...
    if (self.showCloseButton) {
        ...
        [self applyCloseButtonRulesForContentView:content parentView:self.view];
    }
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    [[BANotificationCenter defaultCenter] removeObserver:self name:kBATMessagingMessageDidAppear object:nil];
}
...
- (void)willTransitionToTraitCollection:(UITraitCollection *)newCollection
    ...
    // That's kinda like Android, and may not seem performant, but trust me: recomputing what changed and swapping
    // constraints would be MUCH more complicated
 ...
 */
- (void)applyCloseButtonRulesForContentView:(UIView *)contentView parentView:(UIView *)parentView {
...
- (void)applyAbsolutePositionmentRulesForView:(id<BAMSGStylableView>)view
    ...
    for (NSString *rule in [rules allKeys]) {
        ...
        if ([@"z-index" isEqualToString:rule]) {
            ...
            if ([@"100%" isEqualToString:value] && parentView != nil) {
                ...
                // We need to do that so that autolayout does not go nuts and lets the intrisic content size push the
                // image fullscreen
    ...
    [constraints
        addObjectsFromArray:[NSLayoutConstraint
                                constraintsWithVisualFormat:[NSString
                                                                stringWithFormat:@"|-(>=%f@1000)-[view]-(>=%f@1000)-|",
    ...
    if (!widthApplied) {
        [constraints addObjectsFromArray:
                         [NSLayoutConstraint
                             constraintsWithVisualFormat:[NSString stringWithFormat:@"|-(==%f@600)-[view]-(==%f@600)-|",
                                                                                    margin.left, margin.right]
                                                 options:0
                                                 metrics:nil
                                                   views:NSDictionaryOfVariableBindings(view)]];
...
- (void)applyMainContainerViewRules {
    ...
    [constraints addObjectsFromArray:[NSLayoutConstraint
                                         constraintsWithVisualFormat:
                                             [NSString stringWithFormat:@"|-(>=%f@1000)-[innerView]-(>=%f@1000)-|",
                                                                        margin.left, margin.right]
                                                             options:0
                                                             metrics:nil
                                                               views:NSDictionaryOfVariableBindings(innerView)]];
    ...
    if (!widthApplied) {
        [constraints addObjectsFromArray:[NSLayoutConstraint
                                             constraintsWithVisualFormat:
                                                 [NSString stringWithFormat:@"|-(==%f@600)-[innerView]-(==%f@600)-|",
                                                                            margin.left, margin.right]
                                                                 options:0
                                                                 metrics:nil
                                                                   views:NSDictionaryOfVariableBindings(innerView)]];
    ...
    // Horizontal alignment
    // Special case: if the view is centered and has margin, add a constraint to make it start after the safe
    // area+margin on both sides It might look better if we ignored the margin, but we want to be consistent with
...
- (void)applyContainerViewRulesForNode:(BACSSDOMNode *)node
    ...
    [constraints
        addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:
                                                    [NSString stringWithFormat:@"|-(==%f@800)-[innerView]-(==%f@800)-|",
...
- (void)fillInnerContentView {
    ...
    if (self.imageURL) {
        ...
        BACSSDOMNode *node = [BACSSDOMNode new];
        node.identifier = @"img";
        [self applyAbsolutePositionmentRulesForView:imageView node:node parentView:content];
    }
    ...
    if (self.autoclosingDuration > 0) {
        ...
        [self applyAbsolutePositionmentRulesForView:countdownView node:node parentView:content];
    }
}
...
- (void)addHorizontalCtasToStackView:(BAMSGStackView *)stackView previousView:(UIView *)previousView {
    ...
    for (NSUInteger i = [self.ctas count]; i > 0; i--) {
        BAMSGStackViewItem *ctaItem =
            [self buttonStackViewItemForLabel:self.ctas[i - 1].label
...
- (void)didDetectGlobalTap:(UIGestureRecognizer *)recognizer {
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if (self.globalTapAction != nil) {
            [self.messagingAnalyticsDelegate messageGlobalTapActionTriggered:self.messageDescription
                                                                      action:self.messageDescription.globalTapAction];
            ...
            [BALogger
                publicForDomain:@"Messaging"
                        message:
                            @"Internal error - A global tap action was triggered but something unexpected happened. "
...
} // Autoclosing setup is handled all in this class

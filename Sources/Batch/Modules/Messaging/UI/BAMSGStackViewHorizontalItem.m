//
//  BAMSGStackViewHorizontalItem.m
//  Batch
//
...
@implementation BAMSGStackViewHorizontalItem
...
+ (NSArray<NSLayoutConstraint *> *)constraintsForRules:(BACSSRules *)rules
    ...
    // By default, set a hugging of 1000 on separators and 1 on other views
    [target setContentHuggingPriority:(isSeparatorView ? 1000 : 1) forAxis:UILayoutConstraintAxisHorizontal];
    ...
    BOOL wantsHeightFilled = !isSeparatorView;
        ...
        if ([@"width" isEqualToString:rule]) {
            ...
            if (!isSeparatorView) {
                ...
                if ([@"margin-top" isEqualToString:rule]) {
                    ...
                    // This is only applied when it is the last view, since we would have to read the previous
                    // constraint and add it up Way too much hassle for what we want Oh, by the way: this is negative
                    // too
    ...
    if (wantsHeightFilled) {
        ...
        [constraints addObjectsFromArray:[NSLayoutConstraint
                                             constraintsWithVisualFormat:
                                                 [NSString stringWithFormat:@"V:|-(>=%f@1000)-[target]-(>=%f@1000)-|",
        ...
        if (alignAttr == NSLayoutAttributeTop) {
            alignAttrMargin = margin.top;
        } else if (alignAttr == NSLayoutAttributeBottom) {
            alignAttrMargin = margin.bottom;
        }
    ...
    NSLayoutConstraint *leftMargin =
        [NSLayoutConstraint constraintWithItem:target
                                     ...
                                     attribute:previous ? NSLayoutAttributeRight : NSLayoutAttributeLeft
                                    multiplier:1.0
                                      constant:margin.left];
    // Give the left margin some room to break if the screen is too small
    if (!previous) {
        leftMargin.priority = 800;
    }
    [constraints addObject:leftMargin];
    ...
    if (isSeparatorView) {
        ...
        // Use the separator view to support a right margin. Separator views don't support margin, so we can do that for
        // them If we don't have a next view (which is usually a separator), this will not work. If there's one but it
        // is not a separator we're in for some undefined behaviour!
...
- (NSArray<NSLayoutConstraint *> *)computeConstraints {
    return [BAMSGStackViewHorizontalItem constraintsForRules:self.rules
...
@implementation BAMSGStackViewHorizontalSeparatorItem

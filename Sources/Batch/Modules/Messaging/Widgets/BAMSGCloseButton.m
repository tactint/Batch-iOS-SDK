...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isHidden || !self.enabled || !self.userInteractionEnabled || self.alpha < 0.01) {
        return nil;
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    CGPoint endPoint =
        CGPointMake(CGRectGetMaxX(rect) - _computedGlyphPadding, CGRectGetMaxY(rect) - _computedGlyphPadding);
...
- (void)layoutSubviews {
    ...
    _computedGlyphPadding =
        self.glyphPadding != nil ? [self.glyphPadding floatValue] : scalingRatio * BAMSGCLOSE_GLYPH_BASE_PADDING;
    _computedGlyphWidth =
        self.glyphWidth != nil ? [self.glyphWidth floatValue] : scalingRatio * BAMSGCLOSE_GLYPH_BASE_WIDTH;
    ...
    // We achieve that by making a the path a circle smaller than the outer one, accounting for the stroke expanding
    // half of it's specified width outside of the circle it follows, plus some additional padding for our desired
    // style. It's okay if you don't understand that comment.
    float paddedGlyphWidth = _computedGlyphWidth + 2 * scalingRatio;
    CGRect progressPathFrame = CGRectMake(paddedGlyphWidth / 2, paddedGlyphWidth / 2,
                                          bounds.size.width - paddedGlyphWidth, bounds.size.height - paddedGlyphWidth);

...
- (void)populate {
    dispatch_async([BAUserDataManager sharedQueue], ^{
      ...
      [builtins
          addObject:[BADBGCustomDataAttribute attributeWithName:@"Custom User ID"
                                                          value:[BAParameter objectForKey:kParametersCustomUserIDKey
                                                                                 fallback:@"<None set>"]]];
      ...
      [datasource.tagCollections enumerateKeysAndObjectsWithOptions:NSEnumerationConcurrent
                                                         usingBlock:^(id key, id object, BOOL *stop) {
                                                           BADBGCustomDataTagCollection *tc =
                                                               [BADBGCustomDataTagCollection new];
                                                           tc.collectionName = [@"t." stringByAppendingString:key];

                                                           // A refactoring once turned this untyped "object" into a
                                                           // crasher We now ensure we're gonna put a valid value for
                                                           // tc.tags

...
+ (nullable NSString *)installationID {
    ...
    BAAESB64Cryptor *cryptor =
        [[BAAESB64Cryptor alloc] initWithKey:[NSString stringWithFormat:@"%@XpBXC%iH", BAPrivateKeyStorage, 58]];
    if (![BANullHelper isStringEmpty:di]) {
    ...
    [BAParameter setValue:[NSString stringWithFormat:@"%f", [[NSDate date] timeIntervalSince1970]]
                   forKey:kParametersLocalInstallDateIdentifierKey
                    saved:YES];
...
+ (void)delete {
    ...
    if (![[NSFileManager defaultManager] removeItemAtPath:path error:&err]) {
        [BALogger debugForDomain:@"BAInstallationID"
                         message:@"Could not delete install ID file (path: %@): %@", path, err.localizedDescription];
    }
}

+ (NSString *)filePersistencePath {
    return [[BADirectories pathForBatchAppSupportDirectory]
        stringByAppendingPathComponent:[BABundleIdentifier stringByAppendingString:@".plist"]];
}

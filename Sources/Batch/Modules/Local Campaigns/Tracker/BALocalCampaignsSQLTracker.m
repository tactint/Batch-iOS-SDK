...
- (BOOL)createEventTable {
    BOOL eventTableCreated = [self
        executeSimpleStatement:
            [NSString stringWithFormat:@"create table if not exists %@"
                                        "(campaign_id text not null, kind integer, count integer, last_occurrence "
                                        "integer, unique (campaign_id, kind) on conflict replace);",
                                       TABLE_EVENT]];
...
- (BOOL)prepareStatements {
    ...
    NSString *statement =
        [NSString stringWithFormat:@"INSERT INTO %@ (campaign_id, kind, count, last_occurrence) VALUES (?, ?, ?, ?)",
                                   TABLE_EVENT];
    ...
    statement = [NSString stringWithFormat:@"INSERT INTO %@ (%@, %@) VALUES (?, ?)", TABLE_VIEW_EVENTS,
                                           COLUMN_NAME_VE_CAMPAIGN_ID, COLUMN_NAME_VE_TIMESTAMP];
